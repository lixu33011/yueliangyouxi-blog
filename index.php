...
<head>
    ...
    <title>月亮有喜的博客</title>
    <!-- 移除Video.js相关依赖，引入本地hls.js -->
    <link rel="stylesheet" href="/js/all.min.css">
    <!-- 背景样式 -->
    <link rel="stylesheet" href="<?php echo $homeBackground; ?>">
    <!-- 主页模板样式 -->
...
<body class="bg-container">
    <!-- 动态背景Canvas（按需加载） -->
    <?php if (mb_strpos($homeBackground, 'bg1.css') !== false || mb_strpos($homeBackground, 'bg3.css') !== false): ?>
...
<div class="date-float-container">
    ...
    <div class="personal-info">
        <img src="uploads/<?php echo $personalInfo['avatar']; ?>" alt="头像" class="avatar" onerror="this.src='https://picsum.photos/80/80';">
        <div class="info">
    ...
    <div class="main-layout">
        ...
        <div class="link-container">
            <?php foreach ($imageLinks as $item): ?>
            <a href="<?php echo htmlspecialchars($item['link_url']); ?>" class="link-item" target="_blank">
                <img src="<?php echo htmlspecialchars($item['thumb_url'] ?: $item['image_url']); ?>" alt="<?php echo htmlspecialchars($item['title']); ?>" onerror="this.src='https://picsum.photos/400/200';">
                <div class="link-title"><?php echo htmlspecialchars($item['title']); ?></div>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="article-corner">
        ...
        <div style="text-align:center; padding: 10px;">暂无文章</div>
        <?php else: ?>
        <?php foreach ($articles as $article): ?>
        <div class="article-item">
            <a href="article-detail.php?id=<?php echo intval($article['id']); ?>"><?php echo htmlspecialchars($article['title']); ?></a>
        </div>
        <?php endforeach; ?>
        <?php endif; ?>
        ...
        function playVideo(url) {
            ...
            if (trimUrl.endsWith('.m3u8')) {
                if (typeof Hls === 'undefined') {
                    toggleVideoLoading(false);
                    alert('HLS播放插件加载失败，请检查本地/js/hls.min.js文件！');
                    return;
                }
                ...
                if (Hls.isSupported()) {
                    ...
                    hls.on(Hls.Events.ERROR, function(event, data) {
                        ...
                        if (!data.fatal && retryCount < MAX_RETRY) {
                            retryCount++;
                            console.log(`缓冲异常，第${retryCount}次重试...`);
                            setTimeout(() => playVideo(url), 1000 * retryCount);
                            return;
                        }
                        toggleVideoLoading(false);
                        alert(`视频播放失败：${data.reason || '未知错误'}`);
                    });
                    ...
                    hls.on(Hls.Events.MANIFEST_PARSED, function() {
                        ...
                        setTimeout(() => {
                            video.play().catch(err => {
                                console.log('自动播放失败，需手动点击播放:', err);
                            });
                        }, 1000);
                    });
                ...
                video.addEventListener('error', function() {
                    toggleVideoLoading(false);
                    alert('MP4视频加载失败，请检查地址是否有效！');
                }, { once: true });
            } else {
                toggleVideoLoading(false);
                alert('暂不支持该视频格式，请配置m3u8或MP4地址！');
            }
        }
...
function getLunarDate(date) {
    ...
    function getLeapMonthDays(y) {
        let lm = getLeapMonth(y); // 修复：定义 lm 变量
    ...
    function getLunarMonthDays(y, m) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 初始化背景
            <?php if (mb_strpos($homeBackground, 'bg1.css') !== false): ?>
            initBg1();
            <?php elseif (mb_strpos($homeBackground, 'bg3.css') !== false): ?>
            initBg3();
            <?php endif; ?>
        });
    </script>
